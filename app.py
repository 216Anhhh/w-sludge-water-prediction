...
with tab1:
    ...
    with tab4:
        ...
        if not st.session_state.model_trained:
            ...
            target_eval = st.selectbox("选择目标变量", available_y, format_func=lambda x: y_names_cn.get(x, x),
                                       key='eval')
            ...
            if target_eval:
                ...
                ax.set_title(f'{y_names_en.get(target_eval, target_eval)} - R² = {r2:.4f}', fontsize=13,
                             fontweight='bold', color=text_color)
                ax.legend(loc='upper left', facecolor=colors['plot_face'], edgecolor=colors['border'],
                ...
                target_violin = st.selectbox("选择变量查看分布", available_y + available_X[:4],
                                             format_func=lambda x: y_names_cn.get(x,
                                                                                  x) if x in y_names_cn else x_names_cn.get(
                                                 x, x),
                                             key='violin')
    ...
    st.markdown("---")
    st.markdown(
        f"💧 **污水处理智能分析平台 v6.0** | 4模型轻量版 | {'🌙 暗色模式' if st.session_state.theme == 'dark' else '☀️ 明亮模式'}")
